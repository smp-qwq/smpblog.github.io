...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | SMP&LMC")[0];
}
...
function createtime1() {
  var grt = new Date("01/05/2024 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by SMP&LMC %c 你正在访问SMP&LMCの小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| SMP&LMC") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('SMP&LMC的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
